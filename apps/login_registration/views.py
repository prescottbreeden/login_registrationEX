...
from apps.login_registration.models import Super_User
...
def index(request):
	if 'super_user_id' not in request.session:
		...
		return render(request, 'login_registration/index.html', {'super_user': Super_User.objects.get(id=request.session['super_user_id'])})
